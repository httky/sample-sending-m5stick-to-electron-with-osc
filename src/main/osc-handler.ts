...
// 192.168.200で始まるIPアドレスを優先してローカルのIPアドレスを取得 (なければ最初のIPアドレスを使用)
const localIP = getPreferredLocalAddress('192.168.200')
...
export class OscHandler {
	 ...
	 * @param values
	 */
	#onSend = (_: IpcMainInvokeEvent, host: string, port: number, address: string, values: string[] | number[]): void => {
